  ...
  methods: {
    ...
    login () {
      this.$refs.loginFormRef.validate(async valid => {
        ...
        console.log(res)
        if (res.flag === 0) return this.$message.error('登录失败！')
        this.$message.success('登录成功')

        console.log(this.role)
        if (this.loginForm.role === 'Teacher') {
          this.$router.push('/teacher')
          window.sessionStorage.setItem('token', res.teacherinfo[0].id)
        } else if (this.loginForm.role === 'Admin') {
          this.$router.push('/admin')
          window.sessionStorage.setItem('token', res.userinfo[0].id)
        } else {
          this.$router.push('/student')
          window.sessionStorage.setItem('token', res.studentinfo[0].id)
        }
      })
    },
    async register () {
      const { data: res } = await this.$http.post('auth/register', this.loginForm)
      console.log(res)
      if (res.flag === 0) return this.$message.error('注册失败！')
