          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button style="padding: 3px 0"
                         type="text"
                         @click="deleteBtnT(scope.$index, scope.row)">删除</el-button>
              <el-button style="padding: 3px 0"
                         type="text"
                         @click="showEditDialogT(scope.$index, scope.row)">编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
...
export default {
  ...
  methods: {
    ...
    // 编辑按钮-教师
    async showEditDialogT (index, row) {
      // 根据id获取用户信息
      const { data: res } = await this.$http.post('auth/getuserid', { id: row.id, role: row.role })
      console.log(res)
      this.editForm = res.teacherinfo[0]
      this.editDialogVisibleT = true
    },
    // 重置修改的表单
    editDialogClosed () {
